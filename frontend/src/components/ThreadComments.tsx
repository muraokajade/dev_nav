...
export const ThreadComments: React.FC<Props> = ({ type, refId, category }) => {
  ...
  return (
    <section className="bg-zinc-900 rounded-xl p-6 my-8 shadow-lg max-w-3xl text-zinc-100">
      <h3 className="font-bold mb-3 text-lg text-zinc-200">{category.toUpperCase()}</h3>
            ...
            {editingId === m.id ? (
              <>
                ...
                <div className="mt-2 flex gap-2">
                  <button className="px-3 py-1 bg-green-600 rounded text-white" onClick={() => handleUpdate(m.id)}>
                    保存
                  </button>
                  <button className="px-3 py-1 bg-zinc-700 rounded text-white" onClick={() => setEditingId(null)}>
                    キャンセル
                  </button>
                </div>
              </>
              ...
              <div>
                ...
                <div className="text-xs text-zinc-400 mt-1">
                  {m.updatedAt ? `更新: ${m.updatedAt}` : `作成: ${m.createdAt}`}
                </div>

                {/* 自分の投稿だけ操作表示（email大小文字を無視） */}
                {myEmail && m.userId?.toLowerCase() === myEmail.toLowerCase() && (
                  <div className="mt-1 flex gap-2">
                    ...
                    <button className="text-red-400 underline" onClick={() => handleDelete(m.id)}>
                      削除
                    </button>
                  </div>
                )}
