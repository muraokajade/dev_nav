      ...
      {[...Array(5)].map((_, idx) => {
        ...
        return (
          <span key={idx} className="relative">
            {/* 半分クリック用：左側（0.5） */}
            <span
              className={`absolute inset-y-0 left-0 w-1/2 z-10 ${onChange && !disabled ? "cursor-pointer" : ""}`}
              style={{ display: "inline-block", height: size, width: size / 2 }}
              onClick={() => handleClick(idx, true)}
            />
            {/* 右側クリック用（1.0） */}
            <span
              className={`absolute inset-y-0 right-0 w-1/2 z-10 ${onChange && !disabled ? "cursor-pointer" : ""}`}
              style={{ display: "inline-block", height: size, width: size / 2 }}
            ...
            {isFull ? (
              // ★
              <svg width={size} height={size} fill="currentColor" style={{ color: "gold" }} viewBox="0 0 16 16">
              ...
              // ☆
              <svg width={size} height={size} fill="currentColor" style={{ color: "#d1d5db" }} viewBox="0 0 16 16">
...
export const ReviewScore: React.FC<{ articleId: number; myUserId: number }> = ({ articleId, myUserId }) => {
  const { scores, myScore, loading, submitScore, average, error } = useReviewScores(articleId, myUserId);
  ...
  return (
    <section className="bg-zinc-900 rounded-xl p-6 my-8 shadow-lg max-w-md text-zinc-100">
